...
function SharkWalk(parts) {
  ...
  var bends={},pressed={},contacts=0,cliff=-100,impacts=[],impactClock=0,lastImpact={};
    ...
    for(var sub=0;sub<2;sub++){
      ...
      joints.forEach(function(j){var kind=equipment[j.name];
        ...
        if(kind==='wheel'){
          ...
          var torque=(pressed[j.name]?30000*Math.max(-1,Math.min(1,1-relative/24)):0)-35*relative;
      ...
      points.forEach(function(p){p.incoming=(p.y-p.py-(p.x-p.px)*slope(p.x))/h/Math.hypot(1,slope(p.x));p.wasTouching=p.touching;p.touching=false;});
      contacts=0;
      for(var n=0;n<18;n++){
        ...
        points.forEach(function(p){if(p.x<cliff){if(p.wheel)p.touch=null;return;}if(p.wheel){wheelContact(p,h);if(n===17&&p.touch)contacts++;return;}var g=ground(p.x)-p.r;if(p.y>g){
          p.touching=true;
          // 接地し続ける重みでは鳴らさず、新しい衝突の法線方向の運動量で判定する。
          var kind=equipment[p.group],impulse=p.incoming/p.w;
          if(!p.wasTouching&&(kind==='arm'||kind==='leg')&&impulse>95&&impactClock-(lastImpact[p.group]||-1)>.18){impacts.push({group:p.group,kind:kind,strength:Math.min(1,impulse/380)});lastImpact[p.group]=impactClock;}
          p.y=g;if(n===17)contacts++;p.px=p.x-(p.x-p.px)*.12;p.py=p.y;}});
      }
  ...
  return {takeImpacts:function(){var result=impacts;impacts=[];return result;},balloons:balloons,rear:rear,front:front,feet:feet,hands:hands,knees:knees,elbows:elbows,points:points,joints:joints,ground:ground,set:set,update:update,now:now};
}
