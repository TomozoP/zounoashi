...
左右で位置を決める → 角度と高さを決めて離す → タップでパワーを止めて投球。赤い投球線そのものが1.8秒周期で伸び縮みし、長いほど強く投げる。独立したゲージや文字は出さない。上限付近ほど強く加速し、最大初速は以前の約2.5倍。
球は画面下から一球ずつ届き、停止後に投球位置へ移る。受け取りレーンは置かない。球名と投球線は到着後に出る。
10投の倒した本数を合計し、結果更新時だけスコア欄を二秒表示する。

## 作り

- `physics.js`：cannon-es による三次元の剛体計算。基本の固定刻みは毎秒180回。高速時は半径に応じてさらに細分化し、衝突のすり抜けを防ぐ。
...
球は位置決めまで不透明。角度決めとパワー調整中は半透明にし、投球時に不透明へ戻す。
