...
var ZBowlingScene=(function(){
  ...
  Scene.prototype.screenPosition=function(x,y){var v=new THREE.Vector3(x/540*2-1,1-y/this.logicalH*2,.5).unproject(this.camera),dir=v.sub(this.camera.position).normalize(),t=(.65-this.camera.position.z)/dir.z;return {x:-(this.camera.position.x+dir.x*t),y:this.camera.position.y+dir.y*t};};
  ...
  Scene.prototype.draw=function(ctx,W,H,data,kind,phase,elapsed,position,aim,dt,power,heightOffset){
    ...
    this.ballMeshes.forEach(function(m){m.visible=false;});var m=this.ballMeshes[kind.skin];m.visible=true;m.traverse(function(part){if(!part.material)return;var translucent=phase==='angle'||phase==='power';if(part.material.transparent!==translucent){part.material.transparent=translucent;part.material.needsUpdate=true;}part.material.opacity=translucent?.45:1;part.material.depthWrite=!translucent;part.castShadow=!translucent;});m.scale.setScalar(kind.r);m.quaternion.identity();
    ...
    else m.position.set(position,ZBowlingPhysics.releaseHeight(kind)+heightOffset,.65);
