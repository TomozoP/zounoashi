...
var ZBowlingScene=(function(){
  ...
  Scene.prototype.screenPosition=function(x,y,height){var v=new THREE.Vector3(x/540*2-1,1-y/this.logicalH*2,.5).unproject(this.camera),dir=v.sub(this.camera.position).normalize(),t=(height-this.camera.position.y)/Math.min(-.001,dir.y);return {x:-(this.camera.position.x+dir.x*t),z:this.camera.position.z+dir.z*t};};
  ...
  Scene.prototype.draw=function(ctx,W,H,data,kind,phase,elapsed,position,aim,dt,power,positionZ){
    positionZ=positionZ==null?.65:positionZ;
    ...
    else m.position.set(position,ZBowlingPhysics.releaseHeight(kind),positionZ);
    this.aimLine.visible=phase==='position'||phase==='angle'||phase==='power';
    if(this.aimLine.visible){var length=phase==='power'?positionZ+(30-positionZ)*power:30,vertices=[];for(var z=positionZ;z<length;z+=.85){var end=Math.min(z+.48,length),x=position+aim*(z-positionZ),ex=position+aim*(end-positionZ),w=.055;vertices.push(x-w,.025,z,x+w,.025,z,ex+w,.025,end,x-w,.025,z,ex+w,.025,end,ex-w,.025,end);}this.aimLine.geometry.dispose();this.aimLine.geometry=new T.BufferGeometry();this.aimLine.geometry.setAttribute('position',new T.Float32BufferAttribute(vertices,3));}
