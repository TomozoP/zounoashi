...
function SharkWalk() {
  var points=[], bends={leg:0,arm:0}, pressed={leg:false,arm:false}, contacts=0;
  ...
  function update(dt){
    for(var sub=0;sub<2;sub++){
      ...
      for(var n=0;n<14;n++){
        ...
        feet.forEach(function(f,i){var b=bends.leg,x=-43+80*b+i*9,y=135-75*b;motor(rear,f,x*c-y*s,x*s+y*c,.024);distance(rear,f,Math.hypot(x,y),.07);});
        hands.forEach(function(f,i){var b=bends.arm,x=-38+78*b+i*9,y=135-82*b;motor(front,f,x*c-y*s,x*s+y*c,.022);distance(front,f,Math.hypot(x,y),.07);});
  ...
  return {rear:rear,front:front,feet:feet,hands:hands,points:points,ground:ground,knee:knee,set:set,update:update,now:function(){return{x:(rear.x+front.x)/2,y:rear.y,leg:bends.leg,arm:bends.arm,legPressed:pressed.leg,armPressed:pressed.arm,contacts:contacts,finite:points.every(function(p){return Number.isFinite(p.x)&&Number.isFinite(p.y);})};}};
}
