...
function SharkWalk(parts) {
  ...
  names.forEach(function(name,i){var a=i<2?rear:front,c=i<2?feet[i]:hands[i-2],length=i<2?76:74;
    if(equipment[name]!=='human'){c.r=equipment[name]==='wheel'?28:20;c.wheel=equipment[name]==='wheel';c.y=-c.r;c.py=c.y;}
  ...
  function bend(j,h){
    ...
    var target=j.sign*(suspension?2.3:Math.PI-.18-bends[j.name]*2.1);
    ...
    if(suspension){
      ...
      var torque=Math.max(-90000,Math.min(90000,-45000*error-1800*rate));
