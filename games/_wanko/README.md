...
- **20個目から、ときどき2台まとめて出る**（`DUAL_FROM`）。左からサブ女将が入ってきて、
  左右に1台ずつ並べる。2台とも同時に燃えていて、**どちらも触れる**（画面の左半分は左の台、
  右半分は右の台。キーなら ↑↓ / Tab で持ちかえ）。2台のときは 0.6 倍の大きさで置き、
  導火線は2.2倍。指の距離感が変わらないよう、当たり判定は大きさで割って合わせている
- 台は `bombs` の配列で持つ。1台ずつ `serve → live → pass` と進み、`withBomb()` で
  「いまの1台」に差し替えてから、その台の仕掛けを呼ぶ。画面の点は `toLocal()` で台の中の座標に直す
