...
function shot(angle,dt,omega){var g=new P.Game();g.launch(angle,omega||16);var gap=0;while(!g.finished()){g.step(dt);g.joints.forEach(function(j){gap=Math.max(gap,j.bodyA.pointToWorldFrame(j.pivotA).distanceTo(j.bodyB.pointToWorldFrame(j.pivotB)));});g.bodies.forEach(function(b){assert(Number.isFinite(b.position.x+b.position.y+b.position.z));});}assert(gap<.08,'関節が離れない');return {score:g.pins.filter(function(p){return p.down;}).length,gap:gap,g:g};}
