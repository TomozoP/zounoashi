...
const wheels=make(all('wheel'));groups.forEach(k=>wheels.set(k,true));for(let i=0;i<600;i++)wheels.update(1/60);assert(wheels.now().x>1000,'車輪が地面を蹴って進む');
const air=make(all('wheel'));air.points.forEach(p=>{p.y-=2000;p.py-=2000;});let start=center(air);groups.forEach(k=>air.set(k,true));for(let i=0;i<30;i++)air.update(1/60);assert(Math.abs(center(air)-start)<.001,'空中で車輪の駆動力を加えない');
