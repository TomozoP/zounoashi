...
let passiveHeight;
for(const active of [false,true]){const w=make(all('balloon'));w.points.forEach(p=>{p.y-=3000;p.py=p.y;});const x=center(w),y=height(w);groups.forEach(k=>w.set(k,active));for(let i=0;i<120;i++)w.update(1/60);if(active)assert(height(w)<y-50,'風船の浮力で胴体ごと上がる');assert(w.balloons.leftLeg.r>(active?35:0)&&w.balloons.leftLeg.r<(active?38:16),'風船は非操作時にしぼむ');if(!active)assert(Math.abs(center(w)-x)<.001,'非操作時の風船は横向きの力を加えない');if(active)assert(height(w)<passiveHeight-100,'押すと浮力が強まる');else passiveHeight=height(w);}
// 風船の追加の力は、画面右ではなく尾から頭への向きに従う。
for(const inverted of [false,true]){const w=make(all('balloon'));w.points.forEach(p=>{if(inverted)p.x=445-p.x;p.y-=3000;p.px=p.x;p.py=p.y;});const x=center(w);groups.forEach(k=>w.set(k,true));for(let i=0;i<60;i++)w.update(1/60);assert(inverted?center(w)<x-1:center(w)>x+1,'風船の推進方向は胴体の向きに従う');}
