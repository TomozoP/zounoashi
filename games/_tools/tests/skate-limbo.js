...
g.key(' ');g.step(20);assert.equal(g.probe.now().state,'intro');g.key(' ',true);assert.equal(g.probe.now().state,'play');g.step(1);assert.equal(g.probe.now().target,-1,'開始を反る入力にしない');
...
for(const slope of [-.12,.12]){for(const side of [-1,1]){const trial=running();trial.x=side*Math.sign(slope);trial.bend=.8;trial.speed=7;trial.gates=[{z:5,height:1.5,slope,passed:false,hit:false,drop:0}];trial.goal=6.5;drive(trial,.8,200);assert.equal(trial.reason,side<0?'bar':'clear');}}
...
const afterGoal=running();afterGoal.state='result';afterGoal.reason='clear';afterGoal.z=afterGoal.goal;afterGoal.distance=afterGoal.goal;const goalTime=afterGoal.t;for(let i=0;i<180;i++)P.step(afterGoal,1/60);assert(afterGoal.z>afterGoal.goal+15);assert(afterGoal.t>goalTime+2.9);assert.equal(afterGoal.distance,afterGoal.goal);console.log('ゴール後の時間と滑走継続、記録の固定を確認');
