...
const upright=running(),bent=running();upright.gates.forEach(g=>g.z+=1000);bent.gates.forEach(g=>g.z+=1000);drive(upright,0,180);drive(bent,1,180);assert(upright.speed>bent.speed+1.2,'立った姿勢では十分に速くなる');assert(upright.speed<6.46&&bent.speed>3.4&&bent.speed<3.7,'速度は急激に変えず範囲内に収める');console.log('3秒後の速度：直立 '+upright.speed.toFixed(2)+'m/s、反り '+bent.speed.toFixed(2)+'m/s');
