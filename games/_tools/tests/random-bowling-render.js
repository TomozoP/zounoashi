...
var child,timer;
var hook=`window.__visual={appearance:function(){if(view3d.red.parts.head.children.length!==1||view3d.blue.parts.head.children.length!==1)throw Error('顔や髪が残っている');},tracking:function(){if(view3d.follow>22.001||Math.abs(view3d.followX)>2.001)throw Error('カメラが追い過ぎる');},hold:function(){down(270,600);for(var i=0;i<180;i++)update(1/60);draw();if(omega<13)throw Error('長押しで最大回転へ加速しない');},fit:function(){var oldH=H,oldAngle=angle;for(var h of [780,960,1200,1700]){H=h;view3d.resize(540,h);for(var a of [0,Math.PI/2,Math.PI,-Math.PI/2]){angle=a;draw();for(var p of ZWrestlePhysics.swingPose(a)){var screen=view3d.project(p.x,p.y,p.z);if(screen.x<8||screen.x>532||screen.y<20||screen.y>h-20)throw Error('回転するレスラーが画面外に出る');}}}H=oldH;angle=oldAngle;layout();draw();},side:function(){angle=.35;draw();}};`;
