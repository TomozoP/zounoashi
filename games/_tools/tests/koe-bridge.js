...
let p=g.probe.now();assert.equal(p.state,'play');assert(Math.abs(p.x-13800)<.01);assert(p.road.length<90,'古い道を保持し続けない');assert(jumps>0,'波から飛び出す');assert(p.bestHeight>10,'ジャンプの高さを記録');assert(p.flips>0,'空中回転を着水時に記録');assert.equal(p.obstacles.length,0);assert(Number.isFinite(p.y));
