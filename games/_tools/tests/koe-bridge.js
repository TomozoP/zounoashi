...
g.press(' ');let jumps=0;for(let n=0;n<3600;n++){const pen=g.probe.now().x+270;const next=Math.max(0,Math.round((pen-600)/500));g.dbg.pitch(next%2?400:100);g.step(1);if(g.probe.now().airborne)jumps++;}
