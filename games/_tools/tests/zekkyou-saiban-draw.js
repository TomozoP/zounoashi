...
for(const h of [700,1130,1700]){t.setup(h);await save('start-'+h+'.png');t.win();p.step(30);if(!p.now().won)throw Error('大声で勝訴しない');p.step(100);if(p.now().state!=='result')throw Error('結果が出ない');await save('win-'+h+'.png');}
await fetch('/__done',{method:'POST',body:'法廷・勝訴・録画元の画面を3種類で確認'});
...
server.listen(0,'127.0.0.1',()=>{
  ...
  child=cp.spawn(exe,['--headless=new','--no-first-run','--no-default-browser-check','--enable-unsafe-swiftshader','--window-size=430,930','--user-data-dir='+path.join(out,'profile'),'http://127.0.0.1:'+server.address().port+'/games/_zekkyou-saiban/index.html?recorder-cli=1'],{stdio:'ignore',windowsHide:true});
