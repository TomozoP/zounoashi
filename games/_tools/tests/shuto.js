...
for(const shape of [[375,812],[700,700],[500,1600]]){
 ...
 for(let round=0;round<9;round++){
  ...
  else if(round%3===1){g.probe.step(48);s=g.probe.now();const p=s.people[s.attacker];g.tap(p.x,p.y);assert.equal(g.probe.now().outcome,'caught');assert.equal(g.probe.now().resultText,'見逃さなかった（2.00秒）');}
...
console.log('OK 24fps・手刀1コマ・0.4秒停止後1.1秒で倒れる・結果の秒数');
