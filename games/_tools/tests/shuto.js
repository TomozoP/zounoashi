...
// 成功の表示中は流れる前の入力を受けず、リプレイを見せ終えた4.6秒後に回数を保って進む。
...
for(let n=1;n<=3;n++){
 ...
 for(let i=0;i<45;i++)streak.dbg.tick();assert.equal(streak.probe.now().state,'success');
...
console.log('OK 3連続成功・表示中の連打・4.6秒の境界・失敗後の回数・再挑戦で0に戻る');
