...
// 成功の表示中は流れる前の入力を受けず、次へを押したときだけ回数を保って進む。
...
for(let n=1;n<=3;n++){
 ...
 for(let i=0;i<45;i++)streak.dbg.tick();assert.equal(streak.probe.now().state,'success','押すまで進まない');
 streak.press(' ');assert.equal(streak.probe.now().state,'play');assert.equal(streak.probe.now().score,n);
...
console.log('OK 3連続成功・表示中の連打・次へで進む・失敗後の回数・再挑戦で0に戻る');
