/* 長押し・赤レスラーの向き・関節・大型ピン・10投の完走を確認する。 */
...
g.esc();for(var shot=0;shot<10;shot++){ready();g.key(' ');for(var i=0;i<500;i++){tick(1);if(now().swingTime>2&&P.throwDirection(now().angle).z>.99&&Math.abs(P.throwDirection(now().angle).x)<.08)break;}g.key(' ',true);ready();assert.equal(now().shot,shot+1);}
