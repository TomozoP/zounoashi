...
assert.equal(now().phase,'ready');assert.equal(now().human.length,11);var idleY=now().human[2].y;tick(15);assert.notEqual(now().human[2].y,idleY,'待機中もラグドール');g.down(270,600);tick(5);g.up();assert.equal(now().phase,'ready','短いタップは投げない');assert.equal(now().shot,0);g.down(270,600);tick(30);var slow=now().omega;assert(slow<5.4,'序盤はゆっくり加速する');tick(150);assert(now().omega>12.8&&now().omega<12.9);assert(now().omega>slow);var before=now().angle;g.moveTo(40,100);assert.equal(now().angle,before);assert.equal(now().human.length,11);
for(var i=0;i<100&&P.throwDirection(now().angle).z<.99;i++)tick(1);
var direction=P.throwDirection(now().angle);var angle=now().angle;g.up();assert.equal(now().phase,'flight');assert.equal(now().human.length,11);var pelvis=now().human[0];assert(Math.abs(pelvis.vx-direction.x*(3+57.5*Math.pow(now().omega/16.94,1.5)))<.001);assert(Math.abs(pelvis.vz-direction.z*(3+57.5*Math.pow(now().omega/16.94,1.5)))<.001);
...
g.esc();g.down(270,600);tick(10);g.wrap.fire('pointercancel',{});assert.equal(now().phase,'ready');assert.equal(now().human.length,11);
g.esc();g.key(' ');tick(150);assert.equal(now().phase,'swing');g.key(' ',true);assert.equal(now().phase,'flight');ready();
g.esc();for(var n=0;n<60;n++){g.pad({press:true});g.step(1);}assert.equal(now().phase,'swing');g.pad({});g.step(1);assert.equal(now().phase,'flight');
