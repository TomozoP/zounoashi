...
assert.equal(now().phase,'ready');g.down(270,600);tick(30);var slow=now().omega;assert(slow<5.4,'序盤はゆっくり加速する');tick(150);assert(now().omega>15&&now().omega<15.1);assert(now().omega>slow);var before=now().angle;g.moveTo(40,100);assert.equal(now().angle,before);assert.equal(now().human.length,11);
for(var i=0;i<100&&P.throwDirection(now().angle).z<.99;i++)tick(1);
var direction=P.throwDirection(now().angle);var angle=now().angle;g.up();assert.equal(now().phase,'flight');assert.equal(now().human.length,11);var pelvis=now().human[0];assert(Math.abs(pelvis.vx-direction.x*(3+67*Math.pow(now().omega/20,1.5)))<.001);assert(Math.abs(pelvis.vz-direction.z*(3+67*Math.pow(now().omega/20,1.5)))<.001);
