...
var direction=P.headDirection(now().human);var angle=now().angle;g.up();assert.equal(now().phase,'flight');assert.equal(now().human.length,11);var pelvis=now().human[0];assert(Math.abs(pelvis.vx-direction.x*(12+now().omega*2.7))<.001);assert(Math.abs(pelvis.vz-direction.z*(12+now().omega*2.7))<.001);
...
g.esc();for(var shot=0;shot<10;shot++){ready();g.key(' ');for(var i=0;i<500;i++){tick(1);if(now().swingTime>2&&P.headDirection(now().human).z>.99&&Math.abs(P.headDirection(now().human).x)<.08)break;}g.key(' ',true);ready();assert.equal(now().shot,shot+1);}
...
console.log('長押し加速・最大回転・離す方向・入力取消・ジョイパッド・10投完走・共有・画面6通り：問題なし');
