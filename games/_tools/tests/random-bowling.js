...
tick(3);assert(now().simulationTime>.010&&now().simulationTime<.016,'投げた瞬間は30％速');tick(10);assert.equal(now().playbackRate,1,'すぐ通常速度に戻る');g.down(270,600);g.up();assert.equal(now().phase,'flight');ready();assert.equal(now().shot,1);
