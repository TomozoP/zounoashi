...
assert.equal(now().phase,'intro');g.down(270,600);g.up();assert.equal(now().phase,'intro','開始前は投げない');g.probe.reset();assert.equal(now().phase,'ready');g.down(270,600);tick(30);var slow=now().omega;assert(slow<5.4,'序盤はゆっくり加速する');tick(150);assert(now().omega>12.8&&now().omega<12.9);assert(now().omega>slow);var before=now().angle;g.moveTo(40,100);assert.equal(now().angle,before);assert.equal(now().human.length,11);
