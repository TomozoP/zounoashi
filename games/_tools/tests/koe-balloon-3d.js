...
const hook="window.__check={\nscene:function(){return scene3d;},\nsetup:function(height){H=height;canvas.width=540;canvas.height=H;newRound();draw();},\nstart:function(){state=S.INTRO;draw();},\necho:function(){history=Array.from({length:30},function(_,i){return {id:i+1,end:6,y:.5,worldX:2075,frames:Array.from({length:360},function(){return {y:.48+Math.sin(i)*.07,air:1};})};});T=2.1;replayStep(0);traps=[trapPosition(0,2.1)];hazards=[0,1,2].map(function(i){var h=hazardPosition(i,T);h.x=200+i*110;return h;});draw();},\ncrater:function(){gates=[{x:320,top:300,bottom:H-100,holes:[{x:29,y:100/H,r:50}]}];draw();},\ncrash:function(){popBalloon();draw();}\n};\r\n";
