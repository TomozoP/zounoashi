...
await save('blow.png');
t.crash();check(p.now().state==='scream','衝突時に断末魔の時間がない');p.step(55);check(p.now().state==='fall','衝突後に落下しない');check(scene.flames.every(f=>!f.outer.visible),'衝突後も噴射している');
const before=p.now().doll[0].y;p.step(20);check(Math.abs(p.now().doll[0].y-before)>1,'爆散した部品が動かない');await save('fall.png');
p.step(18);check(p.now().state==='play','1.5秒後に再開しない');
