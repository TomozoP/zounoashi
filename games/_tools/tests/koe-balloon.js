...
g.key(' ',true);g.step(100);assert(g.probe.now().vy<0);
...
for(const shape of [[390,844],[700,700],[500,1600]]){
 ...
 for(let n=0;n<3600;n++){
  ...
  const gate=p.gates.find(x=>x.height+110>p.altitude),target=gate?(gate.left+gate.right)/2:270;
  const l=p.x>target+7,r=p.x<target-7;
  if(l!==left){g.key('ArrowLeft',!l);left=l;}if(r!==right){g.key('ArrowRight',!r);right=r;}g.step(1);
 }
 assert(g.probe.now().score>=15);assert(g.probe.now().scroll>1000);console.log('60秒の上昇・通過',shape.join('×'),g.probe.now().score);
}
