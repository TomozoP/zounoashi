...
window.addEventListener('load',async function(){
 try {
  ...
  var start=performance.now();p.step(23);shot();p.step(47);shot();p.step(47);shot();p.step(40);shot();
...
var server=http.createServer(function(req,res){
 if(req.url==='/__save'||req.url==='/__fail'){
  var parts=[];req.on('data',d=>parts.push(d));req.on('end',function(){
   ...
   console.log(target);console.log('157コマの実描画: '+Math.round(data.ms)+'ミリ秒');finish(0);
  });return;
 }
