...
window.addEventListener('load',async function(){
 try {
  ...
  p.step(133);press();press();
  var start=performance.now();p.step(70);shot();p.step(140);shot();p.step(140);shot();p.step(110);shot();
...
var server=http.createServer(function(req,res){
 if(req.url==='/__save'||req.url==='/__fail'){
  var parts=[];req.on('data',d=>parts.push(d));req.on('end',function(){
   ...
   console.log(target);console.log('460コマの実描画: '+Math.round(data.ms)+'ミリ秒');finish(0);
  });return;
 }
